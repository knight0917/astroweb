      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        ...
        <div className="lg:col-span-7 flex flex-col gap-6">
          ...
          <div className="glass-panel p-4 md:p-6 rounded-2xl border border-slate-800 shadow-2xl bg-slate-950/85 space-y-6">
            ...
            {/* 7 Classical Grahas x 4 Directions Master Matrix */}
                ...
                <table className="w-full text-left text-xs border-collapse font-mono">
                  <thead>
                    <tr className="border-b border-slate-800 text-slate-400 bg-slate-950/80">
                      <th className="p-3 font-bold">Planet (Graha)</th>
                      <th className="p-3 text-center">East (H1, 5, 9)</th>
                      <th className="p-3 text-center">South (H2, 6, 10)</th>
                      <th className="p-3 text-center">West (H3, 7, 11)</th>
                      <th className="p-3 text-center">North (H4, 8, 12)</th>
                      <th className="p-3 text-center text-amber-400 font-extrabold">🏆 Best Direction</th>
                      <th className="p-3 text-left">Ideal Life Application</th>
                    </tr>
                  </thead>
                    ...
                    {Object.values(avResult.directionalAnalysis.planetDirections).map((p) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <td className={`p-3 text-center ${p.bestDirection === "South" ? "bg-amber-500/20 text-amber-300 font-extrabold rounded" : "text-slate-300"}`}>
                            {p.south}
                          </td>
                          <td className={`p-3 text-center ${p.bestDirection === "West" ? "bg-amber-500/20 text-amber-300 font-extrabold rounded" : "text-slate-300"}`}>
                            {p.west}
                          </td>
                          <td className={`p-3 text-center ${p.bestDirection === "North" ? "bg-amber-500/20 text-amber-300 font-extrabold rounded" : "text-slate-300"}`}>
                            {p.north}
                          </td>

                          <td className="p-3 text-center">
                            <span className="px-2.5 py-1 rounded-lg bg-amber-500/20 border border-amber-500/50 text-amber-300 font-extrabold">
                              {p.bestDirection} ({p.bestBindus} pts)
                            </span>
                          </td>
