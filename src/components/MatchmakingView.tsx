  ...
  return (
    <div className="flex flex-col gap-6 animate-in fade-in duration-200">
      ...
      <div className="glass-panel p-4 md:p-6 rounded-2xl border border-slate-800 shadow-2xl flex flex-wrap items-center justify-between gap-4 bg-slate-950/80">
        ...
        {/* Quick Guna Score & Destiny Gate Pills */}
        <div className="flex flex-wrap items-center gap-3">
          <div className="flex items-center gap-3 bg-slate-900/90 px-4 py-2 rounded-2xl border border-slate-800 shadow-inner">
            <div className="text-right">
              <span className="text-[10px] text-slate-400 font-bold uppercase tracking-wider block">Ashtakoota Score</span>
          ...
          <div
            className={`flex items-center gap-2.5 px-4 py-2 rounded-2xl border font-bold text-xs shadow-lg transition-all ${
              matchResult.destinyVerdict.decisionTier === "DESTINED_AUSPICIOUS"
                ? "bg-emerald-950/80 border-emerald-500/80 text-emerald-200 shadow-emerald-950/50"
                : matchResult.destinyVerdict.decisionTier === "PERMISSIBLE_WITH_REMEDIES"
                ? "bg-amber-950/80 border-amber-500/80 text-amber-200 shadow-amber-950/50"
                : "bg-rose-950/90 border-rose-500 text-rose-200 shadow-rose-950/70"
            }`}
          >
            <span className="text-base">
              {matchResult.destinyVerdict.decisionTier === "DESTINED_AUSPICIOUS" ? "💍" : matchResult.destinyVerdict.decisionTier === "PERMISSIBLE_WITH_REMEDIES" ? "⚖️" : "⛔"}
            </span>
            <div>
              <span className="text-[9px] uppercase tracking-wider text-slate-400 block font-semibold">Classical Destiny Gate</span>
              <span>{matchResult.destinyVerdict.decisionTitle.split(" (")[0]}</span>
            </div>
          </div>
        </div>
      </div>

      {/* Grand Marriage Destiny Gate Banner */}
      <div
        className={`p-5 rounded-2xl border shadow-2xl space-y-2 transition-all ${
          matchResult.destinyVerdict.decisionTier === "DESTINED_AUSPICIOUS"
            ? "glass-panel bg-gradient-to-r from-emerald-950/80 via-slate-950/90 to-teal-950/80 border-emerald-500/60 shadow-emerald-950/30"
            : matchResult.destinyVerdict.decisionTier === "PERMISSIBLE_WITH_REMEDIES"
            ? "glass-panel bg-gradient-to-r from-amber-950/80 via-slate-950/90 to-orange-950/80 border-amber-500/60 shadow-amber-950/30"
            : "glass-panel bg-gradient-to-r from-rose-950/90 via-slate-950 to-red-950/90 border-rose-500/80 shadow-rose-950/50"
        }`}
      >
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <span className="text-[10px] font-bold uppercase tracking-widest text-slate-400 block">
                Vedic Matrimonial Destiny Decision • विवाह निर्णय पीठिका
              </span>
              <h3 className={`text-base md:text-lg font-black ${
                ...
                {matchResult.destinyVerdict.decisionTitle}
              </h3>
            </div>
          </div>

          <div className="flex items-center gap-2">
            <span className="text-xs text-slate-400 font-mono">Raw Ashtakoota:</span>
            <span className="text-xs font-mono font-bold text-amber-400 bg-slate-900/90 px-2.5 py-1 rounded-lg border border-slate-800">
              {matchResult.totalScore} / 36 Gunas ({matchResult.percentage}%)
            </span>
          </div>
        </div>

        <p className="text-xs md:text-sm text-slate-200 leading-relaxed font-sans pt-1">
          {matchResult.destinyVerdict.summaryReasoning}
        </p>

        {matchResult.destinyVerdict.fatalImpediments.length > 0 && (
          <div className="mt-3 p-3.5 rounded-xl bg-rose-950/60 border border-rose-600/50 space-y-2">
            <div className="flex items-center gap-2 text-rose-200 font-bold text-xs">
              <span>⚠️</span>
              <span>Fatal Classical Veto Active ({matchResult.destinyVerdict.fatalImpediments.length} Found — Overrules Numerical Gunas):</span>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-xs">
              {matchResult.destinyVerdict.fatalImpediments.map((imp, idx) => (
                <div key={idx} className="bg-slate-950/80 p-2.5 rounded-lg border border-rose-900/60 space-y-1">
                  <div className="flex items-center justify-between text-rose-300 font-bold">
                    <span>{imp.doshaName}</span>
                    <span className="text-[10px] text-rose-400/80 font-mono">{imp.rule}</span>
                  </div>
                  <p className="text-[11px] text-slate-300 leading-tight">{imp.consequence}</p>
                  <p className="text-[10px] text-amber-300/90 font-mono">📖 {imp.shastricSource}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        {matchResult.destinyVerdict.mitigatingFactors.length > 0 && (
          <div className="mt-2 text-[11px] text-emerald-300/90 flex flex-wrap gap-x-3 gap-y-1">
            {matchResult.destinyVerdict.mitigatingFactors.map((m, idx) => (
              <span key={idx}>✓ {m}</span>
            ))}
          </div>
        )}
      ...
      {/* Classical Marriage Destiny Gate & Fatal Vetoes Card */}
      <div className="glass-panel p-6 rounded-2xl border border-rose-500/30 bg-slate-950/80 shadow-2xl space-y-4">
        <div className="flex flex-wrap items-center justify-between gap-2 border-b border-slate-800 pb-3">
          <div className="flex items-center gap-2">
            <span className="text-lg">📜</span>
            <div>
              <h3 className="text-sm font-bold text-rose-300 uppercase tracking-wider">
                Classical Marriage Destiny Gate & Fatal Vetoes (विवाह निर्णय एवं शास्त्रीय निषेध)
              </h3>
              <p className="text-[11px] text-slate-400 font-sans">
                Authoritative Shastric verification from Muhurta Chintamani, Prasna Marga, Brihat Parashara Hora Shastra & Dr. B.V. Raman
              </p>
            </div>
          </div>
          <span
            className={`px-3 py-1 rounded-xl text-xs uppercase font-black border ${
              matchResult.destinyVerdict.isMarriageAdvised
                ? "bg-emerald-950 text-emerald-300 border-emerald-500/50"
                : "bg-rose-950 text-rose-300 border-rose-500/50"
            }`}
          >
            {matchResult.destinyVerdict.isMarriageAdvised ? "✓ Marriage Advised (ग्राह्य)" : "⛔ Marriage Forbidden (वर्ज्य)"}
          </span>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3 text-xs">
          {/* 1. Rajju Koota */}
          <div className="bg-slate-900/70 p-3.5 rounded-xl border border-slate-800 space-y-1.5 flex flex-col justify-between">
            <div>
              <div className="flex items-center justify-between">
                <span className="text-slate-400 text-[10px] uppercase font-bold">1. Rajju Koota (रज्जु)</span>
                <span
                  className={`text-[10px] px-1.5 py-0.5 rounded font-bold ${
                    matchResult.rajju.isDosha
                      ? matchResult.rajju.isMarriageForbidden
                        ? "bg-rose-950 text-rose-300 border border-rose-700"
                        : "bg-amber-950 text-amber-300 border border-amber-700"
                      : "bg-emerald-950 text-emerald-300 border border-emerald-700"
                  }`}
                >
                  {matchResult.rajju.isDosha ? (matchResult.rajju.isMarriageForbidden ? "Fatal Dosha" : "Dosha") : "Pure (निर्दोष)"}
                </span>
              </div>
              <span className="font-bold text-slate-200 block text-xs mt-1">
                Groom: {matchResult.rajju.boyRajju}
              </span>
              <span className="font-bold text-slate-200 block text-xs">
                Bride: {matchResult.rajju.girlRajju}
              </span>
              <p className="text-[10px] text-slate-300 mt-1 leading-tight font-sans">
                {matchResult.rajju.doshaEffect}
              </p>
            </div>
            <span className="text-[9px] text-amber-400/90 font-mono block pt-1 border-t border-slate-800/80">
              Prasna Marga: 'न रज्जुदोषे विवाहः'
            </span>
          </div>
          ...
          <div className="bg-slate-900/70 p-3.5 rounded-xl border border-slate-800 space-y-1.5 flex flex-col justify-between">
            <div>
              <div className="flex items-center justify-between">
                <span className="text-slate-400 text-[10px] uppercase font-bold">2. Vedha Koota (वेध)</span>
                <span
                  className={`text-[10px] px-1.5 py-0.5 rounded font-bold ${
                    matchResult.vedha.isDosha
                      ? "bg-rose-950 text-rose-300 border border-rose-700"
                      : "bg-emerald-950 text-emerald-300 border border-emerald-700"
                  }`}
                >
                  {matchResult.vedha.isDosha ? "Vedha Dosha" : "No Vedha (निर्दोष)"}
                </span>
              </div>
              <span className="font-bold text-slate-200 block text-xs mt-1">
                {matchResult.vedha.boyNakshatra} ⮂ {matchResult.vedha.girlNakshatra}
              </span>
              <p className="text-[10px] text-slate-300 mt-1 leading-tight font-sans">
                {matchResult.vedha.description}
              </p>
            </div>
            <span className="text-[9px] text-amber-400/90 font-mono block pt-1 border-t border-slate-800/80">
              Muhurta Chintamani: 'वेधदोषे कुलक्षयः'
            </span>
          </div>
          ...
          <div className="bg-slate-900/70 p-3.5 rounded-xl border border-slate-800 space-y-1.5 flex flex-col justify-between">
            <div>
              <div className="flex items-center justify-between">
                <span className="text-slate-400 text-[10px] uppercase font-bold">3. Stree Deergha (स्त्री दीर्घ)</span>
                <span
                  className={`text-[10px] px-1.5 py-0.5 rounded font-bold ${
                    matchResult.streeDeergha.isAuspicious
                      ? "bg-emerald-950 text-emerald-300 border border-emerald-700"
                      : "bg-amber-950 text-amber-300 border border-amber-700"
                  }`}
                >
                  {matchResult.streeDeergha.tier}
                </span>
              </div>
              <span className="font-bold text-slate-200 block text-xs mt-1">
                Distance: {matchResult.streeDeergha.nakshatraDistance} Stars
              </span>
              <p className="text-[10px] text-slate-300 mt-1 leading-tight font-sans">
                {matchResult.streeDeergha.description}
              </p>
            </div>
            <span className="text-[9px] text-amber-400/90 font-mono block pt-1 border-t border-slate-800/80">
              Kalaprakasika & Raman Ch. IX
            </span>
          </div>

          {/* 4. Upapada Lagna Match */}
          <div className="bg-slate-900/70 p-3.5 rounded-xl border border-slate-800 space-y-1.5 flex flex-col justify-between">
            <div>
              <div className="flex items-center justify-between">
                <span className="text-slate-400 text-[10px] uppercase font-bold">4. Upapada Lagna (UL)</span>
                <span
                  className={`text-[10px] px-1.5 py-0.5 rounded font-bold ${
                    matchResult.upapadaMatch.isAfflicted
                      ? "bg-rose-950 text-rose-300 border border-rose-700"
                      : "bg-emerald-950 text-emerald-300 border border-emerald-700"
                  }`}
                >
                  {matchResult.upapadaMatch.mutualRelationship.split(" ")[0]}
                </span>
              </div>
              <span className="font-bold text-slate-200 block text-xs mt-1">
                Groom UL: {matchResult.upapadaMatch.boyULSign}
              </span>
              <span className="font-bold text-slate-200 block text-xs">
                Bride UL: {matchResult.upapadaMatch.girlULSign}
              </span>
              <p className="text-[10px] text-slate-300 mt-1 leading-tight font-sans">
                {matchResult.upapadaMatch.description}
              </p>
            </div>
            <span className="text-[9px] text-amber-400/90 font-mono block pt-1 border-t border-slate-800/80">
              BPHS Ch. 30 & Jaimini Sutras
            </span>
          </div>
        </div>

        {/* Shastric Jurisprudence Rule Note */}
        <div className="p-3 bg-slate-900/90 rounded-xl border border-slate-800 text-xs text-slate-300 leading-relaxed font-sans space-y-1">
          <div className="font-bold text-amber-300 flex items-center gap-1.5">
            <span>⚖️</span>
            <span>Classical Vivaha Shastra Law (विवाह शास्त्र नियम):</span>
          </div>
          <p className="text-[11px] text-slate-300">
            {matchResult.destinyVerdict.finalVerdictText}
          </p>
        </div>
      </div>
