...
export type AstroConsultationIntent =
  | "all"
  | "career"
  | "marriage"
  | "name_phonetics"
...
export function detectConsultationIntent(query: string, activeCategory?: string): AstroConsultationIntent {
  ...
  if (activeCategory === "panchang" || /\b(panchang|tithi|muhurta|rahu kalam|abhijit|sunrise|sunset|nakshatra today|hora|choghadiya|festival)\b/.test(q)) {
    return "panchang_muhurta";
  }
  if (activeCategory === "remedies" || /\b(remedy|remedies|mantra|gemstone|puja|upay|parihara|donation|chakra|rudraksha|sadhana|totke|lal kitab)\b/.test(q)) {
    return "remedies_health";
  }
...
export function buildAstroDossier(
  ...
  intent: AstroConsultationIntent = "all"
): string {
  ...
  const lines = [
    ...
    "",
    "#### 📜 1B. JANMA PANCHANGA (NATIVE'S 5 CELESTIAL LIMBS AT BIRTH — जन्म पञ्चाङ्ग):",
    `- **1. Janma Tithi (जन्म तिथि):** **${natalEphemeris.panchanga?.tithi?.name || "N/A"}** (${natalEphemeris.panchanga?.tithi?.paksha || ""} Paksha, #${natalEphemeris.panchanga?.tithi?.index || 1}) • Progress: ${natalEphemeris.panchanga?.tithi?.progressPercent?.toFixed(1) || 0}%`,
    `- **2. Janma Vara (जन्म वार / Birth Weekday):** **${natalEphemeris.panchanga?.vara?.name || "N/A"}** (Day Lord / Ruling Planet: **${natalEphemeris.panchanga?.vara?.lord || "N/A"}**)`,
    `- **3. Janma Nakshatra (जन्म नक्षत्र):** **${natalEphemeris.panchanga?.nakshatra?.sanskritName || natalEphemeris.planets.Moon?.nakshatra?.sanskritName || "N/A"} (Pada ${natalEphemeris.panchanga?.nakshatra?.pada || natalEphemeris.planets.Moon?.nakshatra?.pada || 1})** • Lord: **${natalEphemeris.panchanga?.nakshatra?.lord || natalEphemeris.planets.Moon?.nakshatra?.lord || "N/A"}** • Deity: ${natalEphemeris.panchanga?.nakshatra?.deity || natalEphemeris.planets.Moon?.nakshatra?.deity || "N/A"}`,
    `- **4. Janma Nitya Yoga (जन्म योग):** **${natalEphemeris.panchanga?.yoga?.name || "N/A"}** (Yoga #${natalEphemeris.panchanga?.yoga?.index || 1})`,
    `- **5. Janma Karana (जन्म करण):** **${natalEphemeris.panchanga?.karana?.name || "N/A"}** (Karana #${natalEphemeris.panchanga?.karana?.index || 1})`,
    natalEphemeris.panchanga?.masa ? `- **Janma Masa (वैदिक मास):** **${natalEphemeris.panchanga.masa}**` : "",
    "",
  ...
  // INTENT-BASED SELECTIVE DOSSIER SLICING (4x LATENCY & TOKEN OPTIMIZATION)
  if (intent === "career") {
    lines.push(
      "",
  ...
  } else if (intent === "remedies_health") {
    lines.push(
      "",
  ...
  if (matchmakingSummary && (intent === "all" || intent === "marriage")) {
    lines.push("");
